...
This version (using the commom filsystem) assumes that the input file to
be read is already divided into numFrag parts (proporcional to number of
cores available). Each of them will have the suffix "_number".
For example, if the user submit a execution with the input file 'input.csv'
and numFrag equal to 4, it means that the application will look for the
files 'input.csv_0', 'input.csv_1', 'input.csv_2' and 'input.csv_3'.
"""
...
def preprocessing(grids, window_time, filename, target):
    """Read the input file and create a partial binary matrix."""
    import json
    ncols = len(grids)
...
if __name__ == '__main__':
    ...
    partial_grid = [preprocessing(grids, window_time,
                                  "{}_{}".format(filename, f), city)
